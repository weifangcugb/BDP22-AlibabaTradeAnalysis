...
//        joinDf.show();
        ...
        Dataset<Row> consumeDF = userPayDF.join(shopInfoDF,"shopId").filter("cate3Name = '奶茶' or cate3Name = '中式快餐'")
                .groupBy(col("cate3Name")).agg(max("perPay").alias("maxPay"));
        Dataset<Row> top5PopularMilkTea = userPayDF.join(shopInfoDF,"shopId").filter("cate3Name = '奶茶' or cate3Name = '中式快餐'")
                .filter("cityName = '北京' or cityName = '上海' or cityName = '广州' or cityName = '深圳'");
        String max = consumeDF.select(col("max")).toString();
        top5PopularMilkTea.show();
